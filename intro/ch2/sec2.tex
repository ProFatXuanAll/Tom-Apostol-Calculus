...
\begin{defn}[\iindex{set membership}]\label{defn:1.2.2}
  ...
  Listing the members of a set within braces is sometimes referred to as \textbf{\iindex{the roster notation}}.
\end{defn}

\begin{defn}[\iindex{set equality}]\label{defn:1.2.3}
  Two sets \(A\) and \(B\) are said to be \textbf{\iindex{equal}} (or identical) iff they consist of the same elements, in which case we write \(A = B\).
  If one of the sets contains an element not in the other, we say the sets are \textbf{\iindex{unequal}}, and we write \(A \neq B\).
\end{defn}
